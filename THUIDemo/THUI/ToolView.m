...
- (void)addToolBarView{
    ...
    mToolBarView.frame = CGRectMake(0, 0, 50, ScreenHeight-40);
...
- (void)setBtnFrameIsFirst:(bool)isFirst{
    ...
    isFirst = array.count==btnArray.count?isFirst:NO;//判断功能的增减
    if (isFirst&&array.count>0) {
...
- (void)pushSubviewNibName:(UIView *)aSubviewNibName leftName:(NSString *)aleftName titleName:(NSString *)atitleName rightName:(NSString *)arightName{
    ...
    if((mSlideView == nil) ||(mNav == nil)|| (strcmp(object_getClassName(mSlideView), object_getClassName(aSubviewNibName)) != 0))
    {
        self.frame =  CGRectMake(ScreenWidth-50-Push_View_Width+3, 20, 50+Push_View_Width, ScreenHeight-40);
        ...
        mSlideView.frame = CGRectMake(50-3, 0, Push_View_Width, self.frame.size.height);
 ...
 */
-(void)closePushView{
    self.frame = CGRectMake(ScreenWidth-50, 20, 50, ScreenHeight-40);
...
-(void)moreAction:(UIButton*)btn{
    ...
    moreView = [[MoreToolView alloc]initWithFrame:CGRectMake(self.frame.origin.x-200-8, ScreenHeight-220,200,200)];
